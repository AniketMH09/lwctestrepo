trigger BankAccEmailNotificationTrigger on Payment_Source__c (after insert, after update) {
    ...
    //Bypass the Email Service through Custom Label - this trigger will affect, while loading the data from integration profile/users
        ...
        for(Payment_Source__c src: Trigger.new)
        {
            ...
            else if(Trigger.isUpdate && src.Status__c == 'Deleted' && src.Status__c != Trigger.oldMap.get(src.id).Status__c && (!finSuppress.get(src.Finance_Account_Number__c).Fl_Suppress_All_Notifications__c || finSuppress.get(src.Finance_Account_Number__c).Fl_Suppress_All_Notifications__c ==null))
