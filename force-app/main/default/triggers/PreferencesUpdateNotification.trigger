...
/********************** @Update History **********************
@12 July 2019 - Defect RSS 47625 by Capgemini Team :- In Communication Preferences Detail, 
                                                      SMS notification for updates is stamping as  “SMS Communication Preferences Updates”, 
                                                      but that is not the name of notification that is received. SMS Notification subject should match Email subject
/********************** @Update History **********************/
    ...
    if (Trigger.isBefore) {
        for(Communication_Preferences__c cp:trigger.new){            
         if( ( (cp.Payment_Reminders_via_Email__c )
               ...
               || (cp.Account_Status_Updates_via_Email__c)   //Two new check add for new DSX Changes on 09/04/2018
               || (cp.Account_Status_Updates_via_Text__c))
            ...
            {    
                ...
                for(Communication_Preferences__c cp:trigger.new){
                    ...
                    if(!finSuppress.get(cp.Finance_Account_Number__c).Fl_Suppress_All_Notifications__c || finSuppress.get(cp.Finance_Account_Number__c).Fl_Suppress_All_Notifications__c==null) { // Check the Finance Account is not Suppress
                        if((cp.Payment_Reminders_via_Text__c||cp.Payment_Confirmations_via_Text__c || cp.EasyPay_Communications_via_Text__c || cp.Unsubscribe_from_all__c || cp.Account_Status_Updates_via_Text__c) && cp.Text_Number__c!=null){
                                ...
                                else if((!cp.Unsubscribe_from_all__c) && (cp.Text_Number__c != oldCp.Text_Number__c || cp.Payment_Reminders_via_Text__c != oldCp.Payment_Reminders_via_Text__c || cp.Days_Prior_Reminder__c != oldCp.Days_Prior_Reminder__c || cp.Payment_Confirmations_via_Text__c != oldCp.Payment_Confirmations_via_Text__c 
                                                                          || cp.EasyPay_Communications_via_Text__c != oldCp.EasyPay_Communications_via_Text__c || cp.Account_Status_Updates_via_Text__c != oldCp.Account_Status_Updates_via_Text__c)){
                                    if(isHonda  !='AFS' ){
                                ...
                                if(msgTemplate != null && cp.Text_Number__c!=null){
                                    ...
                                    if(!Test.isRunningTest())
                                    {
                                       //Start : Omkar added for the defect 114000 to avaoid recursive execustion of exacttarget service
                                        ...
                                        Task task = new Task();
                                        task.WhatId = cp.Id;
                                        task.Subject = 'SMS: COMMUNICATION PREFERENCES SAVED'; // Changed Subject Line RSS 47625
                                        //task.WhoId = 
