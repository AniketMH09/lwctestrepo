...
export default class makePaymentLWC extends NavigationMixin(LightningElement) {
  ...
  get confirmationMessage() {
    return (this.flags.showOneTimePayment) ? this.label.AHFC_One_Time_Payment_Message : (this.flags.showPayOff) ? this.label.AHFC_PayOff_Payment_Message : (this.flags.showEasyPay) ? this.label.AHFC_EasyPay_Payment_Message : "";
  }
  ...
  getStateParameters(currentPageReference) {
    if (currentPageReference) {
      ...
      if (currentPageReference.state.payOff === true) {
 ...
 */
  validateStdPaymentAmount(event){
    ...
    else if(this.decStdPaymentAmount > this.dbRemainingAmountDue)
    {
    ...
    if(this.decStdPaymentAmount >= this.decPayoffAmount){
...
/** End of Method validateAddnlPricipalAmount by Akash Solanki */
 ...
 */
   validateScheduleOn(event){
    ...
    console.log('Surendra from Validate schedule '+this.datScheduleOn+'--'+typeof(this.datScheduleOn) );
    console.log('Surendra- datScheduleOn '+this.datScheduleOn+'--'+typeof(this.datScheduleOn) );
    console.log('Surendra- datSystemDate '+this.datSystemDate+'--'+typeof(this.datSystemDate) );
    console.log('Surendra- DueDate '+dueDate+'--'+typeof(dueDate) );
    console.log('Surendra- pastDueStatusFlag '+this.pastDueStatusFlag+'--'+typeof(this.pastDueStatusFlag) );
    console.log('Surendra- Maturity Date '+this.userAccountInfo.Maturity_Date__c+'--'+typeof(this.userAccountInfo.Maturity_Date__c) );
    console.log('Surendra- Account Owner '+this.userAccountInfo.Region_Code+'--'+typeof(this.userAccountInfo.Region_Code) );
    console.log('Surendra- DPD '+this.getDPD(dueDate2,schDate));
    
    if(!this.datScheduleOn)
    {
      console.log('Surendra -Entered Empty Schedule Date');
      ...
      console.log('Surendra -Entered Past Schedule Date');
      this.flags.boolShowScheduleOnMsg=false;
      targetPart.setCustomValidity("Scheduled Payment Date cannot be in the past.");
    }
    else if(this.pastDueStatusFlag==false && (this.datScheduleOn < this.userAccountInfo.Maturity_Date__c) && this.userAccountInfo.Region_Code__c  != "NRC"  && this.getDPD(dueDate2,schDate) > 10){
      console.log('Surendra -Current Finance Account with DPD > 10 Days Criteria');
      this.flags.boolShowScheduleOnMsg=false;
      targetPart.setCustomValidity("Scheduled Payment Date cannot be beyond "+this.addDays(dueDate2, 10));
    }
    else if( this.pastDueStatusFlag==true && (this.datScheduleOn < this.userAccountInfo.Maturity_Date__c) && this.userAccountInfo.Region_Code__c  != "NRC"  && this.getDPD(dueDate2,schDate) > 10){
      console.log('Surendra -Past Due Finance Account with DPD > 10 Days Criteria');
      this.flags.boolShowScheduleOnMsg=flase;
      targetPart.setCustomValidity("Scheduled Payment Date cannot be beyond "+this.addDays(dueDate2, 10));
    }
    else if( this.pastDueStatusFlag==true && this.getDPD(dueDate2,schDate) > 10){
      console.log('Surendra -Past Due Finance Account with DPD > 10 Days Criteria');
      this.flags.boolShowScheduleOnMsg=false;
      targetPart.setCustomValidity("Scheduled Payment date cannot be beyond "+this.datSystemDate);
    }
    else if(this.pastDueStatusFlag==false && this.getDPD(dueDate2,schDate) <= 10 && (this.datScheduleOn < this.userAccountInfo.Maturity_Date__c) &&
     this.userAccountInfo.Region_Code__c != "NRC"  && this.flags.boolShowStdPaymentAmountMsg==true && this.getDPD(todayDate,schDate) >182) {
      console.log('Surendra -CFA +10 DPD before Maturity + !NRC + Sch Payments+ 182 from Today Criteria');
      this.flags.boolShowScheduleOnMsg=flase;
      targetPart.setCustomValidity("Scheduled Payment Date cannot be beyond "+this.addDays(todayDate, 182));
    }
    else if(  (this.datScheduleOn > this.userAccountInfo.Maturity_Date__c) &&  this.userAccountInfo.Region_Code__c != "NRC"  ) {
     console.log('Surendra -Schedule Date > Maturity + !NRC Criteria');
     this.flags.boolShowScheduleOnMsg=flase;
     targetPart.setCustomValidity("Scheduled Payment Date cannot be beyond the Maturity Date of "+this.userAccountInfo.Maturity_Date__c);
      ...
      console.log('Surendra -Entered Futured Schedule Date');
...
handleReviewPayment(event){
  ...
  if(!this.decStdPaymentAmount)
    ...
    else if(this.decStdPaymentAmount >= this.decPayoffAmount && this.userAccountInfo.RecordType.Name === 'Retail' && this.strAccountType === 'Balloon')
    {
        this.flags.boolShowStdPaymentAmountMsg = false;
        targetPart.setCustomValidity(AHFC_PaymentGreaterThanPayOff);
    }
    else if(this.decStdPaymentAmount >= this.decPayoffAmount && this.userAccountInfo.RecordType.Name === 'Lease' && this.strAccountType === 'Balloon')
    {
        this.flags.boolShowStdPaymentAmountMsg = false;
        targetPart.setCustomValidity(AHFC_PayAmtGrtrThanPurchaseAmt);
    }
...
validatePrincipalAmountOnReviewClick(){
  ...
  checkPaymentType(event) {
    this.flags.showPaySources = true;
    this.paymentType = event.target.label;
  ...
  displayPrincipalAmountfield() {
    if (this.userAccountInfo != null) {
      if (this.userAccountInfo.Fl_OneTime_Payment_Eligible_Web__c &&
        ...
        this.strAccountType === AHFC_Account_Type_Balloon && this.flags.boolRefinanced
