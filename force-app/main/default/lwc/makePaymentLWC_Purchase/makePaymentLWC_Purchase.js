...
export default class makePaymentLWC extends NavigationMixin(LightningElement) {
  ...
  @track showezpay;
  @track isValidationSuccess;
  @track payoffamt = 0;
  @track payoffScheduledOn;
  @track payoffamt;
  ...
  setFinanceAccData(recData){
    ...
    console.log('recData.serAccRec>>>>>',recData.serAccRec.Account_Type__c);
    this.financeAccData = {sacRecordId:this.sacRecordId,Regular_Monthly_Payment__c:recData.serAccRec.Regular_Monthly_Payment__c,Payoff_Amount__c:recData.decPayoffAmount,isPayOff:this.isPayOff,Region_Code__c:recData.serAccRec.Region_Code__c, dueDate:recData.serAccRec.Next_Due_Date__c};
    console.log(this.financeAccData,'this.financeAccData');
    if(recData.serAccRec.Account_Type__c === 'Lease' || recData.serAccRec.Account_Type__c === 'Balloon'){
    ...
    getPayoffDetails ({
      ...
      if(result){
        ...
        this.payoffamt = recData.serAccRec.Payoff_Amount__c;
  ...
  getManagePayoff() {
    ...
    managePayOff({
      ...
      .then((result) => {
        ...
        this.isPayOff = false;
        console.log('Acc type1'+this.userAccountInfo.Account_Type__c);
        ...
        this.getPaymentSourcesList();
        if(this.enrolledInEasyPayFlag == true){ // US : 4710(Added enroll flag) - Added by Edwin Antony
...
**/ 	
  getEasyPayValues(){	
    ...
    this.isValidationSuccess = this.template.querySelector("c-a-h-f-c_make-payment-easy-pay-l-w-c").boolShowAutoPayAmtMsg; 	
