...
export default class AHFC_fullCalendar extends LightningElement {
    ...
    for(var index = 1; index<=end.getDate(); index++){
      ...
      if(dateInstance.getTime() > todayDate.getTime() && dateInstance.getTime() <= future.getTime()){
        ...
        if(dateVar === todayFormattedDate){
          ...
          this.dates.push({text:index, className : 'disabled', key:index+Math.random(), formatted:dateVar});
        }
